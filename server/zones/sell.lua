Drugs.RegisterZoneLoaded('sell', function(zone)
    ...
    local zoneName, zoneAction = Drugs.GenerateZoneNameAndAction('sell', inputs, outputs)
    local blip = false
    ...
    return {
        ...
        timeToExecute = zone.timeToSell or 3.5,
        type = 'sell',
        inputs = inputs,
        outputs = outputs,
        blip = blip
...
Drugs.RegisterZoneProcessor('sell', function(xPlayer, zoneInfo, cb)
    ...
    elseif (notEnoughItems) then
        ...
        xPlayer.showNotification(_U('item_sold', itemsRemoved))
        xPlayer.showNotification(_U('item_sell', itemsReceived))
        Drugs.UpdateLastTimeTriggerd(xPlayer.source)
...
Drugs.RegisterZoneLabelGenerator('sell', function(zoneInfo)
    local inputItems = zoneInfo.inputs or {}
    local itemLabels = {}
